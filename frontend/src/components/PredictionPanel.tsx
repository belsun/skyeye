...
function ForecastSection({
  ...
  const isUp = primary?.direction === 'up';
  const ns = forecast.news_summary || { total: 0, positive: 0, negative: 0, neutral: 0, top_headlines: [], top_impact: [] };
  const stats = forecast.similar_stats || { count: 0, up_ratio_5d: 0, up_ratio_10d: 0, avg_ret_5d: null, avg_ret_10d: null };
...
function PredictionCard({ label, pred }: { label: string; pred: HorizonPrediction }) {
  ...
  const quality = pred.model_quality || 'unverified';
  const cvLift = pred.model_cv_lift != null
    ? `${pred.model_cv_lift >= 0 ? '+' : ''}${(pred.model_cv_lift * 100).toFixed(1)}pp CV`
    : null;
  const maxContrib = pred.top_drivers.length > 0
    ? Math.max(...pred.top_drivers.map((d) => d.contribution), 0.01)
    : 0.01;
  const gate = pred.signal_gate;
  const failedGateChecks = gate?.checks.filter((check) => !check.ok).slice(0, 2) ?? [];
  ...
  return (
    <div className={`fc-pred-card ${isUp ? 'up' : 'down'}`}>
      ...
      {pred.model_quality_label && (
        <div className="fc-quality-row">
          <span className={`fc-quality-badge ${quality}`}>{pred.model_quality_label}</span>
          {cvLift && <span className="fc-quality-cv">{cvLift}</span>}
        </div>
      )}
      {pred.model_quality_message && (
        <div className="fc-quality-note">{pred.model_quality_message}</div>
      )}
      {gate && (
        <div className={`fc-signal-gate ${gate.status}`}>
          <div className="fc-signal-gate-head">
            <span className={`fc-signal-gate-badge ${gate.status}`}>{gate.label}</span>
            <span>{gate.message}</span>
          </div>
          {failedGateChecks.length > 0 && (
            <div className="fc-signal-gate-checks">
