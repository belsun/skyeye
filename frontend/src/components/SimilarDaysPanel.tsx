...
import { fetchSimilarDays } from '../api';
import type { SimilarDaysData } from '../types';
...
export default function SimilarDaysPanel({ symbol, date, onClose }: Props) {
  ...
  useEffect(() => {
    setLoading(true);
    setError('');
    fetchSimilarDays(symbol, date)
      .then(setData)
      .catch(() => setError('查找相似日期失败'))
          ...
          {data.similar_days.map((day) => (
            <div key={day.date} className="sim-day-card">
              ...
              <div className="sim-day-details">
                ...
                <span className="sim-day-chip neutral">{day.n_articles} 篇新闻</span>
                <span className="sim-day-chip neutral">RSI {day.rsi_14.toFixed(0)}</span>
              </div>
